...
//
// The card, scrim, slide and fillets are DropPanel.qml's; this file is only
// what goes inside.
//
...
DropPanel {
    ...
    onKeyPressed: event => {
        ...
        if (event.key === Qt.Key_Escape && root.inputsOpen) {
            root.inputsOpen = false
            event.accepted = true
        } else if (event.key === Qt.Key_Right) {
            root.setVolume(sink, (sink?.audio?.volume ?? 0) + 0.05)
            event.accepted = true
        } else if (event.key === Qt.Key_Left) {
            root.setVolume(sink, (sink?.audio?.volume ?? 0) - 0.05)
            event.accepted = true
        } else if (event.key === Qt.Key_M) {
            root.toggleMute(sink)
            event.accepted = true
        }
    }
    ...
    Column {
        ...
        Repeater {
            ...
            // SPACING 0, gaps built into the children instead. A positioner
            // drops the spacing around a child the moment its height reaches
            // 0, unanimated - so the collapsing dropdown would finish its
            // curve and then snap shut by a few more pixels. Same trap as the
            // OSD in Osd.qml.
            delegate: Column {
                id: section
                ...
                // --- dropdown (inputs) -----------------------------------------
                //
                // The current device, standing in for the list until it is
                // asked for.
                ...
                // --- devices --------------------------------------------------
                //
                // Always laid out; a dropdown reveals it by animating the clip
                // height, so the card grows on the same curve as the list
                // rather than jumping.
                ...
                Item {
                    ...
                    // The gap under the dropdown button lives inside this
                    // height, so it closes with the list.
                            ...
                            delegate: Rectangle {
                                ...
                                MouseArea {
                                    ...
                                    onClicked: {
                                        ...
                                        // A choice made closes the dropdown
                                        // it was made in.
